...
(defface improved-highlight-symbol-face
  '((((class color) (background dark))
     (:background "SpringGreen1" ;; "gray30"
		  ))
    (((class color) (background light))
     (:background "SpringGreen1" ;; "gray90"
		  )))
  "Face used by `highlight-symbol-mode'."
  :group 'highlight-symbol)
...
(defun highlight-symbol-add-symbol (symbol)
  (unless (highlight-symbol-symbol-highlighted-p symbol)
    (when (equal symbol highlight-symbol)
      (highlight-symbol-mode-remove-temp))
    (let ((color "SpringGreen1"
		 ;; XXX Thu May 29 03:31:58 2014, Gabriel Laddel
		 ;; Old code:
		 ;; (nth highlight-symbol-color-index
    ...
    (let ((symbol (highlight-symbol-get-symbol)))
      (unless (or (equal symbol highlight-symbol)
        ...
        (when symbol
          ...
          (highlight-symbol-add-symbol-with-face symbol 'improved-highlight-symbol-face)
          (font-lock-fontify-buffer))))))
